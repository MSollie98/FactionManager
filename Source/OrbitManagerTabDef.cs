...
{
	public class OrbitManagerTabDef : ManagerTabDef
	{
		public override void DrawManagerRect(Rect outRect, ref Vector2 scrollPosition, ref float scrollViewHeight)
		{
			...
			List<MapParent> list = Find.World.worldObjects.MapParents.FindAll((MapParent item) => (item != null && item.def.defName == "ClaimableSpaceSite"));
			...
			foreach (MapParent outpost in list)
			{
				...
				OrbitManagerTabDef.DrawOutpostRow(outpost, num, rect2);
				num += 50f;
			}
